...
export type RailpackCacheKey = string;
...
export type OperationKind = "build" | "deploy" | "cert" | "machine_add" | "machine_build_cache_prune" | "machine_update" | "machine_storage_prepare" | "machine_lifecycle" | "core_replace" | "credential_grant" | "network_repair" | "service_restart" | "managed_dns_reconcile" | "ingress_configure" | "namespace_remove" | "volume_create" | "volume_remove";
...
export type FirstMachineInstallArtifacts = { ployzd: InstallArtifactSpec, ebpf_bytecode: InstallArtifactSpec, ebpf_ctl: InstallArtifactSpec, railpack: InstallArtifactSpec,
/**
...
export type StorageEffectFailure = { "kind": "unsupported_platform" } | { "kind": "installation", message: string, } | { "kind": "pool_list", message: string, } | { "kind": "ambiguous_pools", candidates: Array<ZfsPoolName>, } | { "kind": "explicit_pool_absent", pool: ZfsPoolName, } | { "kind": "owned_pool", message: string, } | { "kind": "owned_pool_too_small", total_bytes: number, available_bytes: number, required_headroom_bytes: number, minimum_pool_bytes: number, } | { "kind": "dataset", message: string, } | { "kind": "prepared_state_unavailable", message: string, } | { "kind": "prepared_state_mismatch", message: string, } | { "kind": "gather_parse", message: string, } | { "kind": "quota_shrink", dataset: DatasetName, current: number, requested: number, } | { "kind": "quota_capacity_exceeded", total_bytes: number, provisioned_used_bytes: number, free_bytes: number, required_headroom_bytes: number, requested_total_bytes: number, } | { "kind": "destructive_effect", message: string, } | { "kind": "operation_timed_out" } | { "kind": "process_failed", message: string, };
...
export const OPERATION_API_CONTRACTS = [
  ...
  { name: "volume.create", subject: "plz.v1.rpc.operator.command.volume.create", execution: "accepts_operation", request: "VolumeCreateRequest", success: "AcceptedOperation", error: "VolumeCreateError", response: "VolumeCreateResponse" },
