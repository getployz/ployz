...
Ployz should derive warning-only role visibility from fresh role observations instead of durable membership tables. Known gateways, DNS processes, tunnel processes, and machine agents are role processes with recent observations; stale role processes age out of role observation windows and diagnostics without a cleanup operation.
