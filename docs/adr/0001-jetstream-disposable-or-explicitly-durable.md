...
After JetStream loss, core recovery should be an explicit reindex operation: machines and data-plane roles reconnect or rejoin, publish fresh signed facts from Docker, keeper state, gateway/DNS last-known-good state, certificate authority material, and local role authority, then control rebuilds indexes and adopts only unambiguous state. Ambiguous or missing facts remain observations until a follow-up operation repairs them.
