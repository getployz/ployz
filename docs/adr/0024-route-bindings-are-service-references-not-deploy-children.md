...
> Current v2 amendment: standalone route attach/remove now write binding rows directly. Deploy only creates a missing deterministic automatic binding and does not reconcile a manifest route set. Gateways join the binding's service to the active entry in one Namespace intent document and matching machine endpoint testimony; no serving-target or revision-entry layer remains.

Ployz Route Bindings attach external hostnames to named services, not to deploy manifests or currently-running containers. Gateways resolve a binding's service through current Namespace intent and serve only matching endpoints from its active deploy (ADR 0023).
