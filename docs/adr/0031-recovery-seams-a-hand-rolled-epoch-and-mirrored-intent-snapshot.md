...
- **The one cluster secret a candidate needs is the CA signing key, and it rides
  ...
  (`--recovery-secret`, e.g. Cloud passing its stored value) or generated by keeper
  and shown once for the operator to keep; it wraps the CA key before it is
  persisted or mirrored and is itself never stored. A machine compromise yields
