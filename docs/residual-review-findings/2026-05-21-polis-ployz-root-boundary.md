...
- Polis traits such as `RecordAuthorizer`, `Reducer`, `ProjectionStore`,
  `OperationStore`, and `MutationReceiptStore` are public before production
  adapters exist. They are acceptable for the current proof, but the next real
  adapter slice should either use them directly or narrow/delete unsupported
  extension points.
