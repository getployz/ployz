...
- ployzd: delete src/machine_agent/observer.rs (InMemoryObservationStore, zero usages); delete MachineServiceCallError + its only (test) importer, the three unused api_*_endpoint helpers, and the machine_endpoint_subject identity wrapper in src/services.rs; delete the never-produced NodeContainerRunDomainError::StartedContainerUnhealthy wire variant in src/machine_protocol.rs plus its client/failure plumbing in machine_rpc.rs and deploy_worker/failure.rs; delete src/machine_runtime.rs (46-line identity layer) pointing callers at machine_service_runtime directly
...
The worst single rot, flagged independently by three reviewers. KeeperFirstMachineInstall is a 23-field flattened twin of FirstMachineInstallSpec bridged by ~130 lines of From shuffling — verified that the keeper stdin wire contract is the NESTED spec (ployzctl/src/runtime.rs:416 converts flat back to nested before serializing), so the flat type never crosses a wire. Three byte-identical artifact structs metastasize into hand-built fixtures across 5 crates. NKey minting is implemented 3x backed by a weakly-validated duplicate seed newtype with a literal round-trip conversion in mint.rs.

- Delete KeeperFirstMachineInstall and both From impls from ployz-core/src/install.rs (~200 lines); ployz-keeper/src/cli.rs first_machine_install_target destructures FirstMachineInstallSpec.artifacts directly; ployzctl/src/commands/init.rs FirstMachineInitMode carries FirstMachineInstallSpec and runtime.rs serializes it without conversion; update ployz-core/tests/install_contract.rs
...
The single highest-leverage structural fix. Verified: ployz-test-support/Cargo.toml depends on ployzd via machine.rs imports, so the crates that need shared fixtures most (ployz-core, ployz-nats, ployz-keeper) cannot import it. Result: 47 copies of machine_id, ~14 hand-rolled TestNats wrappers, 5 terminal-status pollers, 9+ hand-rolled JetStream bootstraps drifting from production, triplicated join-template fixtures, 3 shell_quote copies.

- Move ObservingContainerRunner and the ployzd-coupled parts of ployz-test-support/src/machine.rs into ployzd/tests/support/ (its only natural consumers); ployz-test-support becomes a leaf over ployz-core + ployz-nats
...
- Add one connected TestNats fixture (controller/user/machine clients, jetstream context, optional resource bootstrap) replacing the ~14 per-file wrappers and 15 duplicate timeout constants; ployzd/tests/support/control.rs keeps only control-process wiring
...
- Use the existing KV_*/PLZ_* constants in bootstrap.rs manifest(); add MACHINE_CONTAINER_OBSERVATION_PREFIX to ployz-core/src/state.rs next to its siblings and use it in observations.rs
...
**Effort:** medium — **Order:** after CS8-submit-error (error_map.rs contents shrink first; shared operation_api.rs/controllers.rs) — **Risk:** Low-medium — mostly moves; the activation-ownership move is internal rewiring with identical NATS-visible behavior. Verification: ployzd tests (control_runtime, machine_add_mint, api_client_nats, deploy_operation) + DinD e2e first-machine/join path.

Verified at 1533 lines doing five jobs. After CS8 deletes most of the error-translation forest, the split is pure file moves. Also fixes two layering lies: MachineQueryRuntime (the read side) performs cluster-truth writes, and init_first_machine_activate runs a 30-second polling workflow inside a request handler.

- Convert ployzd/src/operation_api.rs to operation_api/{queries.rs, submit.rs, machine_join.rs, first_machine.rs, error_map.rs}; unit tests move with their functions (pure moves)
- Move machine activation (activate_reported_machine) off MachineQueryRuntime onto OperationControllers/machine_join.rs as record-then-activate; the query runtime loses its write method and the first_machine_active_machine field reach-through, becoming genuinely read-only
- Extract the init_first_machine_activate redeem-wait/seed-write/report workflow into first_machine.rs so the handler is a thin trigger (option (b) only — moving it into the mint worker changes the handler's behavior shape and is out of scope for this pinned-behavior pass)
...
### CS10-machine-seam: Unify the machine RPC seam: one envelope, one request model, one machine/ module

**Effort:** large — **Order:** after CS2-dead-rust (machine_runtime.rs/observer.rs/StartedContainerUnhealthy deletions land first) and after CS4 (test-support's machine.rs move); parallel-safe with CS6-CS8 (different files) — **Risk:** Medium — touches the deploy↔machine wire path, but request/response JSON shapes are unchanged (machine_id moves from payload struct to subject parameter only on the in-process port, not the wire). Verification: ployzd machine_rpc/machine_service_runtime/machine_runtime/role_process test suites + DinD e2e deploy path.

The machine surface models everything twice across the seam: a 6x-copied request/validate/unwrap envelope with the responder check duplicated in both match arms, a 3x byte-identical transport-error mapping, and machine_runtime_types.rs whose five request structs differ from machine_protocol.rs twins only by a machine_id field that is routing, not payload (bridged by five field-copying From impls). The machine_* file family carves at accretion order — 'machine_agent' contains no agent.

- Add one generic NodeRpcResponse<T, E> envelope in machine_protocol.rs and one call_node helper doing request_json + responder validation + the NatsJsonServiceRequestError mapping exactly once; the five per-endpoint response enums, eight wrong_responder checks, and two of three error-mapping functions disappear; machine_rpc.rs drops from 779 to ~300 lines
- Delete the request half of machine_runtime_types.rs and all five From impls; the NodeContainerRuntime port (deploy_worker/ports.rs) takes (machine_id: &MachineId, request: NodeXxxRpcRequest); remove the deploy_worker.rs re-export seam
- Fold the family into a machine/ directory: machine/protocol.rs (wire types + envelope), machine/service.rs (server handlers), machine/client.rs (NATS adapters), machine/runner.rs (NodeContainerRunner trait + decide_container_run, absorbing machine_agent/), machine/process.rs (process runtime + observer loop) — pure moves with re-exports
- machine_service_runtime: one bind helper replacing the six 10-line clone-dance blocks; merge the twin created/existing container start-error mappers into one function taking the variant constructor
...
**Effort:** medium — **Order:** after CS10-machine-seam and CS11 (shared machine_process_runtime.rs/control_runtime.rs) — **Risk:** Medium on dataplane_runtime — host provisioning order must be byte-identical. Verification: gated wireguard_dataplane test + local-dataplane-proof.sh Layer A + ployzd role_process/gateway_process_runtime suites; the docker change is pinned by machine_service_runtime and DinD e2e.
...
- Add a small ployzd process-support module: shutdown_signal(), BackoffSchedule { interval, cap }, a generic attempt/health recorder, LazyHandle<T>.get_or_open(); machine_process_runtime, gateway_process_runtime, control_runtime compose from it (~150 lines, three subtly-divergent copies become one)
- Delete LazyLocalDockerManagedContainerRunner (docker/runner.rs:38-196): give DockerManagedContainerRunner an internal lazily-connecting docker() over tokio OnceCell; the duplicate trait impls, both connect_* helpers, and the hardcoded magic network values disappear; machine_process_runtime constructs the real runner (runner.rs drops to ~830 lines)
...
**Effort:** medium — **Order:** after CS3-install-model (FirstMachineInitMode carries the nested spec) and CS4; parallel-safe with CS13 (different crates) — **Risk:** Low — CLI surface is pinned by cli_contract.rs (1115 lines of exact argument/help/exit assertions) and machine_cli_contract/machine_add_binary_nats. Verification: those suites plus DinD e2e init path.
...
- Extract the keeper subprocess machinery (run_keeper_first_machine_install, OutputCapture, CapturedFile, LimitedOutput, LocalKeeperInstallError) into keeper_install.rs — runtime.rs drops to ~450 lines; if CS13's fsx primitive is exposed, CapturedFile reuses it instead of being the third staged-temp-path copy
- Collapse the 12 dispatch arms via one generic call helper or per-command execute(self, api) methods; replace the 20-arm is_first_machine_activation_retryable match with retryability declared on the one variant that can be retryable; collapse the 11 bare-{source} Display arms
