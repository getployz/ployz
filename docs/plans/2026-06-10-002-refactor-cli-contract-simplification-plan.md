...
| R6 | The migration must update scripts, docs, and binary tests that currently expect `ployz-keeper first-node-install` to receive dozens of flags. |
...
- KTD5. Use a clean break for internal first-node install flags: This repository is still a greenfield reset, so the plan favors deleting the flat flag path after internal callers and tests move to specs.
...
sequenceDiagram
  ...
  Ctl->>Core: deserialize and validate first-node install spec
  Ctl->>Keeper: first-node-install --spec -
  Keeper->>Core: deserialize spec and validate domain fields
  Keeper->>Steps: convert spec into FirstNodeInstallTarget
...
- Introduce typed JSON specs for first-node install and shared artifact bundle inputs.
...
- Preserving the old flat `ployz-keeper first-node-install` flag contract as a public compatibility surface.
...
**Approach:** Delete `DaemonRoleParseError` and `parse_role_args` from core. Add a `clap` role parser under `ployzd`, convert it into `DaemonProcessRole`, and keep `first_node_process_set` plus `joined_node_process_set` in core. Move `DaemonProcessRole::command_args()` out of core into keeper's systemd rendering or another keeper-local helper because it exists to render `ExecStart`, not to model product state.
...
**Test Scenarios:**
- `crates/ployzd/tests/role_process.rs`: `control`, `node --id node_7`, `gateway`, and `dns` parse into the same `DaemonProcessRole` values as today.
- `crates/ployzd/tests/role_process.rs`: invalid role values and missing node ids exit with CLI parse failure at the binary boundary.
- `crates/ployz-keeper/tests/systemd.rs`: rendered `ployzd` systemd units still invoke `control`, `node --id <id>`, `gateway`, and `dns` as expected.
...
### U3. Introduce typed first-node install specs

**Goal:** Replace the flat first-node install flag contract with serde-backed DTOs that `ployzctl` and `ployz-keeper` validate through one domain conversion path.
...
**Test Scenarios:**
- `crates/ployz-core/tests/install_contract.rs`: a complete first-node install spec round-trips through JSON and rejects unknown fields.
...
- `crates/ployz-core/tests/install_contract.rs`: `KeeperFirstNodeInstall` no longer exposes shell command rendering or flat argv construction.
- `crates/ployz-keeper/tests/bootstrap.rs`: converting a valid spec produces a first-node install plan with artifact install, NATS config, role environment, and supervisor unit steps.
- `crates/ployz-keeper/tests/local.rs`: optional node public IP and machine join template path survive conversion into the role environment target.

**Verification:** The same install information is represented once as typed data, not as duplicated flat flag staging structs.

### U4. Replace `ployz-keeper first-node-install` flag parsing with a spec loader

**Goal:** Delete the largest hand-written parser by making keeper accept a first-node install spec from a file or stdin.
...
**Approach:** Delete `first_node_install_cli.rs` and replace it with a small keeper-local spec-loading path. The `first-node-install` subcommand accepts `--spec <path>` and `--spec -`; keeper reads JSON, deserializes the core DTO, converts it into `FirstNodeInstallTarget`, then runs the existing install planner. Do not retain the old flat flags behind compatibility branches.

**Patterns to Follow:** `crates/ployzd/src/config.rs` already reads and deserializes `MachineJoinTemplate` from a configured path.

**Test Scenarios:**
- `crates/ployz-keeper/src/cli.rs` unit tests: `first-node-install --spec /tmp/spec.json` parses into a first-node install command carrying a spec source.
- `crates/ployz-keeper/tests/local.rs`: a valid spec file drives the same first-node install behavior that the flat flags previously drove.
...
**Verification:** `crates/ployz-keeper/src/first_node_install_cli.rs` is deleted, and no keeper parser enumerates every artifact flag.
...
**Test Scenarios:**
- `crates/ployzctl/tests/cli_contract.rs`: `init --run-keeper-install --install-spec <path>` invokes keeper with `first-node-install --spec -` and sends valid JSON to stdin.
...
- `crates/ployzctl/tests/init_binary_nats.rs`: first-node activation behavior is unchanged after the parser and spec rewrite.
- `scripts/local-dataplane-proof.sh` and `scripts/hetzner-two-node-acceptance.sh`: generated or expected first-node install commands use the new spec shape.
...
- `crates/ployz-keeper/tests/local.rs`: spec-based first-node install remains covered with valid and invalid specs.
- `crates/ployzd/tests/role_process.rs`: daemon role CLI remains covered at binary or parser boundary.
- Documentation examples use the new spec-based first-node install command shape.
...
- AE1. Given a valid first-node install spec file, when `ployz-keeper first-node-install --spec <file>` runs, then keeper plans the same artifact installs, NATS config, role environment, and supervisor units that the old flat flags planned.
- AE2. Given `ployzctl init --run-keeper-install --install-spec <path>` with valid install input, when it invokes keeper, then the spawned process receives `first-node-install --spec -` and valid JSON on stdin.
- AE3. Given an invalid role invocation such as `ployzd node --id`, when the binary starts, then the failure is reported by the `ployzd` CLI boundary and no core parser is involved.
...
| --- | --- |
| `crates/ployz-keeper/src/first_node_install_cli.rs` | Deleted. Keeper accepts `first-node-install --spec <path|->` only. |
...
| Area | Before | After |
| --- | --- | --- |
| First-node install input | Dozens of flat flags repeated across keeper, ployzctl, tests, and scripts | One typed spec loaded by file or stdin |
...
This refactor changes local executable contracts and tests but should not change NATS service contracts, operation state, Docker behavior, or runtime cluster truth. The main external-facing impact is the first-node install command shape. Because first-node install appears in docs, scripts, and acceptance harnesses, the implementation must update examples and fixtures in the same branch as the parser rewrite.
...
| First-node install script examples drift from the real runtime path | Update `ployzctl --emit` tests, local proof scripts, and docs together with the spec handoff. |
...
- Update operator-facing examples that mention `ployz-keeper first-node-install` flat flags.
- Explain that first-node install specs are control-plane install contracts and can be supplied by file or stdin.
...
- `crates/ployz-keeper/src/first_node_install_cli.rs`: current wide first-node install parser.
