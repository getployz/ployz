...
- R7. User-facing commands are NATS services under stable subjects such as
  `svc.plz.deploy.submit` and `svc.plz.ops.status`.
- R8. Node-local commands are node-scoped services such as
  `svc.plz.node.<node_id>.container.run`; users cannot call them directly.
- R9. KV buckets hold current state only: core truth, observations, operation
  status, and short-lived locks.
- R10. Streams hold operation history, audit history, observation transitions,
  and job trigger subjects.
...
- KTD3. **One daemon binary everywhere.** `ployzd` can run as core,
  controller, edge, gateway, DNS, or combinations by config and credentials.
  Only core nodes run `nats-server`.

- KTD4. **NATS server is managed, not embedded.** Ployz owns config,
  credentials, stream/KV/Object Store bootstrap, and lifecycle. The NATS server
  remains an external process/system service.

- KTD5. **NATS runs over iroh by default for nodes.** Edge `ployzd` starts a
  local loopback NATS forwarder. `async-nats` connects to that local address.
  The forwarder opens an iroh connection to a core `ployzd` tunnel endpoint,
  which forwards bytes to the local core `nats-server` client listener. This
  keeps NATS native while avoiding public NATS exposure and surviving address
  changes.
...
- KTD8. **Mutating services create operations, workers execute operations.**
  `svc.plz.deploy.submit` validates, publishes the submitted event, writes
  `KV_OPS`, and returns. Durable consumers perform the deploy.
...
Every machine can run `ployzd`. Only core nodes run `nats-server`.
...
3-200 nodes:
  ...
  edges = agents, gateway, DNS, runtime
```
...
`PLZ_OPS` is retained operation history and job trigger input. Deploy workers
consume `plz.op.*.deploy.submitted` through durable pull consumers.
...
```text
svc.plz.deploy.submit
svc.plz.deploy.plan
svc.plz.ops.status
svc.plz.ops.watch
svc.plz.ops.cancel
svc.plz.ops.list
svc.plz.service.inspect
svc.plz.service.list
svc.plz.service.remove
svc.plz.service.scale
svc.plz.machine.list
svc.plz.machine.inspect
svc.plz.cert.ensure
```
...
```text
svc.plz.node.<node_id>.inspect
svc.plz.node.<node_id>.container.run
svc.plz.node.<node_id>.container.stop
svc.plz.node.<node_id>.container.remove
svc.plz.node.<node_id>.container.start
svc.plz.node.<node_id>.predeploy.run
svc.plz.node.<node_id>.logs.tail
svc.plz.node.<node_id>.volume.create
svc.plz.node.<node_id>.volume.remove
```
...
- `ployzd` wires processes, credentials, service handlers, controllers, node
  agent, iroh tunnel endpoints, Docker, gateway, DNS.
...
ployz init
  ...
  start core-side iroh NATS tunnel listener
  start ployzd roles: core, controller, edge
...
ployz machine add user@host --name node-2
  ...
  controller SSHes to target once
  installer writes ployzd config and join bundle
  installer starts ployzd
  target starts iroh endpoint
  target starts local loopback NATS tunnel
  target async-nats connects to localhost tunnel
  target registers svc.plz.node.<node_id>.*
  target writes KV_OBS/nodes/<node_id>/heartbeat
  controller requests svc.plz.node.<node_id>.inspect
...
edge ployzd
  async-nats client
    -> 127.0.0.1:<ephemeral>
      -> local NATS tunnel forwarder
        -> iroh QUIC stream
          -> core ployzd tunnel endpoint
...
all behave as normal NATS features.
...
- **Goal:** Let one `ployzd` start or connect to `nats-server`, then bootstrap
  the required JetStream resources.
...
- **Approach:** Pin a minimum NATS server version. Render a local single-node
  config first. Create `KV_CORE`, `KV_OPS`, `KV_OBS`, `KV_LOCKS`,
  `PLZ_OPS`, `PLZ_AUDIT`, `PLZ_OBS_TRANSITIONS`, and Object Store buckets.
...
- **Files:**
  ...
  - `crates/ployz-nats/src/connect.rs`
  - `crates/ployzd/src/iroh_tunnel/mod.rs`
  - `crates/ployzd/tests/iroh_nats_tunnel.rs`
- **Approach:** Core `ployzd` accepts a dedicated iroh protocol for NATS byte
  forwarding and proxies each stream to local `nats-server`. Edge `ployzd`
  starts a local loopback TCP listener and forwards accepted sockets over iroh.
  `async-nats` connects to the loopback address with normal NATS credentials.
  Tunnel state is observable but not cluster truth.
- **Test scenarios:**
...
- **Verification:** `cargo test -p ployz-transport && cargo test -p ployzd iroh_nats_tunnel`
...
- **Approach:** `deploy.submit` publishes a durable submitted event with
  `Nats-Msg-Id`, writes `KV_OPS/ops/<op_id>`, and returns `op_id`. A durable
  pull consumer reads submitted events and moves the operation through a fake
  state machine.
- **Test scenarios:**
...
- **Test scenarios:**
  - Node credential can subscribe/respond only to its node service subjects.
  - Node credential cannot write `KV_CORE` or call another node service.
  - Controller credential can consume deploy jobs and call node services.
  - User credential can call user services and cannot call node services.
...
- **Test scenarios:**
  - Docker event creates or updates `KV_OBS/containers/<node>/<container>`.
...
- **Goal:** Build projections over KV rather than controller-owned runtime
  state.
- **Requirements:** R4, R9, R10, R14
- **Dependencies:** U6, U7
- **Files:**
  - `crates/ployzd/src/gateway/mod.rs`
  - `crates/ployzd/src/dns/mod.rs`
  - `crates/ployzd/src/controllers/cert.rs`
  - `crates/ployzd/tests/gateway_projection.rs`
  - `crates/ployzd/tests/cert_operation.rs`
- **Approach:** Gateway watches `KV_CORE/routes`, `KV_CORE/certs`, and
  `KV_OBS/containers`; it keeps last good config if NATS disappears. Cert
  controller uses `KV_LOCKS/acme/<hostname>`, Object Store encrypted bundles,
  and schedule/job subjects.
- **Test scenarios:**
...
- **Verification:** `cargo test -p ployzd gateway_projection cert_operation`
...
| iroh tunnel down on edge | Existing containers continue; node services unavailable; status shows tunnel down | `crates/ployzd/tests/iroh_nats_tunnel.rs` |
...
- AE5. Gateway continues serving last good routes when NATS is down and reports
  degraded status through observation state.
