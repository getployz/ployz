...
The current alpha proves the control plane, keeper install, NATS auth, machine join, and deploy path, but the user-facing commands still expose proof-harness details. First-node init requires explicit node names and install specs. Machine add requires operation IDs, idempotency keys, node IDs, machine names, and a printed shell command. Deploy requires internal operation fields plus split route flags.
...
- R4. `ployzctl machine init root@your-server-ip` forms a first-node cluster on the remote machine through SSH and records local client context for later commands.
- R5. First-node identity defaults to the remote machine's system hostname; the resulting `NodeId` and `MachineName` are the same value.
...
- R8. Gateway and DNS roles are installed and enabled by default for first-node init and joined machines.
...
- KTD5. Store local client context after first-node init: subsequent commands should load NATS URL, CA path, and operator seed path from a local config before falling back to env vars and `--nats`.
...
sequenceDiagram
  ...
  CLI->>CLI: validate identity and build first-node install spec
  CLI->>SSH: run curl https://ployz.sh | sh in first-node mode
  SSH->>Script: resolve release manifest and install keeper
  Script->>Keeper: first-node-install --spec
  Keeper->>Core: install NATS, control, node, gateway, DNS
  CLI->>SSH: collect NATS material needed by local operator
  CLI->>CLI: write local cluster context
  CLI->>Core: activate first node operation
...
The first-node path should converge the current DinD proof shape into a real product command. The CLI still builds an install spec, but the operator does not see it. The remote machine still runs `ployz.sh` and keeper, but the operator does not copy generated commands by hand.

The joined-node path mirrors first-node orchestration after the cluster exists: read the remote hostname, submit `machine add` with derived identity, receive the join bundle, SSH to the machine, run the real installer join mode, and wait for the operation to complete or fail.
...
- **Test scenarios:** Commands load config when env is absent; env vars override config; missing config gives the current clear connection error; first-node init writes usable config atomically.
...
- **Approach:** Parse `user@host` targets, run bounded SSH commands, read `hostname`, validate it as `NodeId` and `MachineName`, and support `--name` as the only quick-start identity override.
- **Test scenarios:** Valid hostname derives node and machine name; invalid hostname fails before remote install; `--name` overrides hostname; SSH command failures include phase and stderr.
- **Verification:** CLI contract tests cover parsing and error messages without requiring live SSH.

### U4. First-Node `machine init`

- **Goal:** Replace visible first-node install specs with `ployzctl machine init root@host`.
...
- **Approach:** Add `MachineCli::Init`, build the existing first-node install spec internally, run `ployz.sh` on the remote host in first-node mode, collect operator material, write local context, and call the existing first-node activation API.
- **Test scenarios:** The generated install spec includes hostname-derived node ID, gateway install, DNS install, bootstrap URL, release artifacts, and NATS server descriptor; failed keeper install exits non-zero with output; successful init writes local context and activates the first node.
...
- **Approach:** Replace gateway-only booleans with an install role policy that defaults to node, gateway, and DNS. Wire the DNS role runtime so `ployzd dns` is a runnable supervised process instead of returning `RoleRuntimePending`.
- **Test scenarios:** First-node process sets include control, node, gateway, and DNS; joined-node process sets include node, gateway, and DNS; `--no-gateway` and `--no-dns` skip only the requested roles; DNS runtime can start and observe NATS route state.
...
- **Test scenarios:** Remote add submits the expected request; duplicate node fails through operation status; installer failure after token redemption records `MachineAddFailure::BootstrapFailed`; gateway and DNS default into the join bundle.
- **Verification:** DinD two-machine e2e deploys a workload after joining the second node through the new command.
...
- **Test scenarios:** Docs examples are copied into CLI contract tests where possible; a DinD e2e scenario runs first-node init, joined-node add, and deploy with only the documented commands.
...
- AE2. Given a fresh Ubuntu server with hostname `sg-core-1`, when the user runs `ployzctl machine init root@203.0.113.10`, then the first active machine has node ID `sg-core-1`, machine name `sg-core-1`, gateway role, and DNS role.
...
- `crates/ployzctl/src/commands/machine.rs` currently requires explicit node, name, operation ID, idempotency key, and gateway flag for machine add.
...
- `crates/ployz-e2e/tests/support/dind/formation.rs` contains the proof-harness first-node formation flow that `machine init` should encapsulate.
