...
absolute deadline with locally bounded retry-loop SSH attempts; a remote timeout
alone is not sufficient evidence of a bounded wait.
...
- Recovery commands came exactly from `recovery.txt`, with `/` for live recovery
  or a validated `PLOYZ_RECOVERY_MOUNT_ROOT` for rescue recovery: `<yes/evidence>`
