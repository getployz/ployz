# Two-Node Acceptance (retired)

The Hetzner two-node acceptance harness is retired. Its scripts
(`scripts/hetzner-two-node-acceptance.sh`, `scripts/prepare-h0-artifacts.sh`)
predated the direct TLS NATS control plane and invoked daemon commands that no
longer exist.
...
- Acceptance path: the Docker-in-Docker e2e harness covers the same
  two-machine product flow (first-node install, machine add/join, deploy,
