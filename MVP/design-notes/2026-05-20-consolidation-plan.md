...
| Slice 7: Fact substrate and authz concept extraction | Complete | `MVP/p2panda-facts/src/lib.rs` now keeps substrate contracts and fact data carriers; `MVP/p2panda-facts/src/backend.rs` owns the in-memory/SQLite p2panda `LogStore` and `TopicStore` adapter boundary; `MVP/p2panda-facts/src/store_runtime.rs` owns local store open/write/import/rebuild/shared-store lifecycle; `MVP/p2panda-facts/src/sync.rs` owns bidirectional log-sync orchestration and sync event import. Data-carrier accessor boilerplate was replaced with public fields. `MVP/p2panda-authz/src/lib.rs` now keeps membership/authz mutation/replay logic; `identity.rs` owns identifiers and durable key bindings, `store.rs` owns durable membership stores, and `authority_view.rs` owns the snapshot/read model used by fact authorization. `cargo test --manifest-path MVP/Cargo.toml -p mvp-p2panda-facts` and `cargo test --manifest-path MVP/Cargo.toml -p mvp-p2panda-authz` pass. |
| Slice 8: Bus semantic state extraction | Complete | `MVP/bus/src/memory/delivery_runtime.rs` owns bounded worker execution, queue pressure metrics, and inflight/drain accounting. `MVP/bus/src/memory/fact_store.rs` owns in-memory fact writes, reads, payload reads, and the `FactAuthorizer` adapter. `MVP/bus/src/memory.rs` now owns the memory bus routing surface: grants, subscriptions, bridge dispatch preparation, and request/reply planning. Colocated tests were moved under semantically named `MVP/bus/src/memory/tests/` files. `cargo test --manifest-path MVP/Cargo.toml -p mvp-bus` passes. |
| Slice 9: Projection domain reducers | Complete | `MVP/projection/src/reducer/key_expectation.rs` owns the fact-key/payload validation contract, leaving `reducer.rs` focused on one-pass state reduction. The remaining reducer still covers multiple domains, but that coupling is now the projection pass itself: node/service/serving/lease/ACME facts share conflict ordering, active lease lookup, and projection status accounting. Splitting by domain here would mostly move branches without removing the shared reduction rules. `cargo test --manifest-path MVP/Cargo.toml -p mvp-projection` passes. |
| Slice 10: Lease, machine remove, daemon membership, iroh, and sub-threshold god-module audit | Complete | `MVP/lease/src/ledger.rs` owns lease ledger reduction and fact hash rules, leaving `lib.rs` closer to command surface/data types. Lease data carriers now use public fields rather than accessor boilerplate, and ACME/projection callers use that direct data shape. `MVP/node/src/membership/daemon_control_protocol.rs` owns daemon control request/response serialization, and `MVP/node/src/membership/daemon_control.rs` owns control socket runtime, request I/O, status/deploy dispatch, and control task shutdown. Membership is now closer to join/admission/daemon-loop orchestration. `MVP/iroh/src/facts/local_view.rs` owns the local index, rejected-entry tracking, candidate status projection, and `FactSource` adapter; `facts.rs` is now the iroh node/doc write and sync surface. `MVP/machine/src/remove/participant.rs` owns participant prepare/stop RPC subjects, wire encode/decode, timeout use, and response validation; `remove.rs` is now the phased command, fact writer, precondition checks, and result shaping. `MVP/acme/src/lib.rs` remains a cohesive ACME HTTP-01 challenge presentation domain, `MVP/bus/src/actor.rs` remains a cohesive actor facade over `MemoryBus`, and `MVP/machine/src/facts.rs` remains the machine-remove fact protocol/key/payload/recovery validation surface. The workspace test passes. |
| Slice 11: Final consolidation gate | Complete | `find MVP -maxdepth 1 -type d -name '*-p2panda'` prints nothing; the command-adapter suffix `rg` prints nothing; `cargo metadata --manifest-path MVP/Cargo.toml --no-deps --format-version 1` has no suffixed command packages; the LOC audit prints no non-E2E Rust files above 1,500 lines; `cargo test --manifest-path MVP/Cargo.toml --workspace` passes. |
...
- [x] `MVP/commands-p2panda/`, `MVP/deploy-p2panda/`,
  `MVP/machine-p2panda/`, `MVP/environment-p2panda/`, and
  `MVP/routing-p2panda/` do not exist.
- [x] `MVP/Cargo.toml` has no `*-p2panda` workspace members.
- [x] `rg "-p2panda|_p2panda" MVP -g 'Cargo.toml' -g '*.rs'` returns no crate
  suffix references.
- [x] `find MVP -path '*/src/*.rs' -not -path '*/e2e/*' -print0 | xargs -0 wc -l`
  shows no production files over 1,500 LOC.
- [x] `cargo test --manifest-path MVP/Cargo.toml --workspace` passes.
- [x] `MVP/scripts/three-server-smoke.sh` passes.
