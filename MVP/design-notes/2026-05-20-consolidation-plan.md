...
| Slice 9: Projection domain reducers | In progress | `MVP/projection/src/reducer/key_expectation.rs` now owns the fact-key/payload validation contract, leaving `reducer.rs` focused on state reduction. `cargo test --manifest-path MVP/Cargo.toml -p mvp-projection` passes. Remaining audit question: whether per-domain reducer modules would delete meaningful branching or only fragment a cohesive one-pass reducer. |
| Slice 10: Lease, machine remove, daemon membership, iroh, and sub-threshold god-module audit | In progress | `MVP/lease/src/ledger.rs` now owns lease ledger reduction and fact hash rules, leaving `lib.rs` closer to command surface/data types. `cargo test --manifest-path MVP/Cargo.toml -p mvp-lease` passes. Machine remove, daemon membership, iroh, ACME, and other suspicious sub-threshold modules still need cohesive-as-is vs simplification findings. |
