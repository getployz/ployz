...
7. Evaluate p2panda-maintained sync/network plumbing after persistent stores
   exist on both sides.
...
- `p2panda-net` is the preferred long-term network direction when its API line
  can replace bespoke MVP transport. Slice 020 proved the git network stack can
  compile and spawn local log-sync nodes in this workspace; production
  fact-store code remains on stable p2panda 0.5.2 until a deliberate git API
  migration.
...
Slice 020 proved direct `p2panda-sync` over paired in-memory streams and a git
`p2panda-net` compatibility smoke test:

- two stores sync missing operations through p2panda-maintained log sync;
- repeated sync after convergence is a no-op;
- bidirectional same-key conflicts remain reducer-visible candidates;
- trusted-replica egress and writer-grant import checks stay Ployz-owned;
- large-load sync reports exact convergence at 200, 1,000, and 10,000 facts.

The next network substitution slice should bias toward the git `p2panda-net`
