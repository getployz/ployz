...
Slice 032 ran the p2panda-net dependency substitution while the transport
...
`mvp-p2panda-transport` now uses crates.io p2panda `0.5.2` crates, and
`mvp-iroh` is aligned to the compatible iroh `0.96` family. No git-pinned
p2panda dependency remains in the MVP lockfile. The Ployz-facing authority,
projection, and process-serving contracts stayed intact; the only runtime
