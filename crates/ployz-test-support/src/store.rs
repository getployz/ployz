...
impl MachineStore for MemoryStore {
    ...
    async fn subscribe_machines(&self) -> Result<ployz_store_api::MachineSubscription> {
        let mut inner = self.lock_inner();
...
impl DeployCommitStore for MemoryStore {
    async fn apply_deploy_commit(&self, commit: &DeployCommit) -> Result<()> {
        ...
        let namespace = commit.namespace.clone();
        let removed: HashSet<&str> = commit.removed_services.iter().map(String::as_str).collect();
        inner
            .service_releases
            .retain(|(record_namespace, service), _| {
                record_namespace != &namespace || !removed.contains(service.as_str())
            });

        for release in &commit.releases {
            let key = (release.namespace.clone(), release.service.clone());
            inner.service_releases.insert(key, release.clone());
        }

        let active_instance_ids: HashSet<&InstanceId> = commit
            .releases
            .iter()
            .flat_map(|release| {
                release
                    ...
                    .slots
                    .iter()
                    .map(|slot| &slot.active_instance_id)
            })
            .collect();

        inner.instance_status.retain(|_, record| {
            record.namespace != namespace || active_instance_ids.contains(&record.instance_id)
        });
        Self::broadcast_routing_refresh(&mut inner);
        Ok(())
    }
