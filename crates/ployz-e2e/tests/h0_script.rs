...
fn hetzner_h0_script_drives_the_product_path() {
    ...
    assert!(!script.contains("edge_bootstrap_tunnel_listen_addr="));
    ...
    assert!(script.contains("extract_bootstrap_tunnel_command"));
    assert!(script.contains("machine add did not print a bootstrap tunnel command"));
    ...
    assert!(script.contains("deploy_wireguard_ebpf_prepared"));
    assert!(script.contains("deploy_completed"));
    assert!(script.contains("require_deploy_dataplane_evidence"));
    assert!(script.contains("deploy_event_json"));
    assert!(script.contains("deploy_has_event \"$log_file\" deploy_wireguard_ebpf_prepared"));
