...
#[test]
fn mismatched_join_result_is_rejected() {
    let membership = FakeMembership::new(MachineStatus::Absent);
    ...
    assert!(matches!(
        error,
        MachineFailure::MutationMismatch { machine } if machine.as_str() == "node-a"
    ));
    assert_eq!(
        membership.joined(),
...
#[test]
fn stale_absent_observation_accepts_current_matching_join_result() {
    let membership = FakeMembership::new(MachineStatus::Absent);
    ...
    let outcome = service
        .add_machine(&context(), request("node-a", 1, "fd00::1"))
        .expect("already joined by another writer");

    assert!(
        matches!(outcome, MachineAddOutcome::Joined(joined) if joined == membership_record("node-a", 4, "fd00::1"))
    );
    assert_eq!(
        membership.joined(),
        vec![membership_record("node-a", 1, "fd00::1")]
