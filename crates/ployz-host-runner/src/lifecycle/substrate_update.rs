...
struct LegacyMachineJoinMaterial {
    cluster_name: ployz_core::install::MachineJoinClusterName,
    dataplane_endpoint_supernet: ployz_core::network::MachineEndpointSupernet,
    runtime_nats_url: ployz_core::install::MachineJoinRuntimeNatsUrl,
    trusted_nats: ployz_core::install::MachineJoinTrustedNats,
    recovery_key_wrapped: ployz_core::install::WrappedCaKey,
    core_seeds_wrapped: ployz_core::install::WrappedCoreSeeds,
    ployzd: ployz_core::install::InstallArtifactSpec,
    ebpf_bytecode: ployz_core::install::InstallArtifactSpec,
    ebpf_ctl: ployz_core::install::InstallArtifactSpec,
    railpack: ployz_core::install::InstallArtifactSpec,
}

fn promote_machine_join_template_release(
    path: &Path,
    target_version: &ployz_core::install::ExactPloyzVersion,
    ...
    let mut template = match serde_json::from_slice::<ployz_core::install::MachineJoinTemplate>(
        &contents,
    ) {
...
impl LegacyMachineJoinTemplate {
    fn into_current(
        self,
        target_version: ployz_core::install::ExactPloyzVersion,
    ) -> Result<ployz_core::install::MachineJoinTemplate, FailureMessage> {
        ...
        Ok(ployz_core::install::MachineJoinTemplate {
...
mod tests {
    ...
    use super::{
        InstalledUpdateUnit, installed_update_units, promote_machine_join_template_release,
    };
    ...
    fn incoherent_legacy_template_fails_without_replacing_the_file() {
        ...
        *legacy
        ...
        let error = promote_machine_join_template_release(&path, &version)
            .expect_err("incoherent legacy template fails");

        assert!(error.as_str().contains("artifact versions disagree"));
    ...
    fn unknown_legacy_fields_fail_without_replacing_the_file() {
        ...
        promote_machine_join_template_release(&path, &version)
            .expect_err("unknown legacy field fails");
    ...
    fn noncanonical_legacy_railpack_path_fails_without_replacing_the_file() {
        ...
        *legacy
        ...
        let error = promote_machine_join_template_release(&path, &version)
            .expect_err("noncanonical Railpack path fails");

        assert!(error.as_str().contains("Railpack install path"));
        assert_eq!(fs::read(path).expect("read unchanged template"), original);
    }
