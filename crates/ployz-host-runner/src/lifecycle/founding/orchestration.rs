//! Strict check-then-do founding sequence.
...
use super::{FoundingMilestone, FoundingStateDirectory, FoundingStateError};
...
/// response decoding. Host Runner owns when each call is legal.
...
/// Idempotent privileged effects performed around the control-plane boundary.
///
/// Every method is an `ensure`: a crash after an effect but before its durable
/// milestone may repeat the call safely. Gateway and DNS units are installed
/// but remain disabled and inactive until their role implementations ship.
...
pub async fn found_machine_one_with_progress(
    ...
    run_host_step(
        state,
        FoundingMilestone::EndpointSubnet,
        FoundingStep::AllocateEndpointSubnet,
        || host.ensure_machine_endpoint_subnet(),
    )?;
    run_host_step(
        state,
