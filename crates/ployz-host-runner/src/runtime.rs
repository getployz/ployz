...
pub(crate) const PLOYZ_HOST_PORTS_ASSURED_EXTERNALLY_ENV: &str =
    "PLOYZ_HOST_PORTS_ASSURED_EXTERNALLY";
...
pub(crate) const HOST_RUNNER_STATE_DIR: &str = "/var/lib/ployz";
...
pub(crate) fn failure_summary(failure: &HostRunnerPlanFailure) -> &str {
    match failure {
...
pub(crate) fn failure_message(message: &str) -> FailureMessage {
    FailureMessage::try_new(message).expect("generated Host Runner failure message is non-empty")
}
