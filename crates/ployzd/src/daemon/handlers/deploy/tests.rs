...
fn decode_manifest_rejects_invalid_json_with_structured_error() {
    let error =
        decode_manifest("{not-json").expect_err("invalid manifest json should return daemon error");
...
async fn branch_apply_prepared_outcome_allows_retry_from_failed_for_same_prepared_id() {
    ...
    store
        .mark_branch_environment_applying(&Namespace::new("pr-39"), &DeployId::new("prepare-1"), 2)
        .await
        .expect("claim branch environment apply");
    record_branch_apply_prepared_outcome(
        &store,
    ...
    store
        .mark_branch_environment_applying(&Namespace::new("pr-39"), &DeployId::new("prepare-1"), 21)
        .await
...
async fn branch_applying_replay_action_prefers_active_replay_after_race() {
    ...
    store
        ...
        .expect("mark branch environment applying");
    store
        .mark_branch_environment_active(&namespace, &prepared_deploy_id, &prepared_deploy_id, 30)
        .await
...
fn volume_move_result_maps_succeeded_transfer() {
    let result =
        volume_move_result_from_transfer(transfer_payload("succeeded", Some(42), Some(4096), None))
...
fn volume_move_result_keeps_running_transfer_pending() {
    let result = volume_move_result_from_transfer(transfer_payload("running", None, None, None))
...
fn deploy_failure_payload_preserves_baseline_details() {
    ...
    let payload =
        deploy_failure_payload_for_error(&PloyzError::Deploy(DeployError::DeployBaselineChanged {
...
async fn lock_loss_marks_applying_deploy_failed_with_warning() {
    ...
    let preview: DeployPreview = serde_json::from_str(record.summary_json()).expect("preview json");
    assert!(
        preview
            .warnings
            .iter()
