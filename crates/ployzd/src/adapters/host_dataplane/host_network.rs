...
use ployz_core::dataplane::{
    ...
    WireGuardPeer, WireGuardPeerStatus, WireGuardPublicKey, WireGuardRttStatus, WireGuardStatus,
};
#[cfg(target_os = "linux")]
use rtnetlink::{
...
pub(super) async fn read_wireguard_status(
    ...
    let interface_mtu = read_interface_mtu(wg_ifname).await.ok();
    let peers = join_all(
        host.peers
            .into_iter()
            .map(|(key, peer)| async move { peer_status(key.to_string(), peer, probe).await }),
    )
    .await
    .into_iter()
    .collect::<Result<Vec<_>, _>>()?;
...
async fn read_interface_mtu(ifname: &str) -> Result<u32, String> {
    ...
    link.attributes
        ...
        .ok_or_else(|| format!("interface {ifname} had no MTU"))
}
