...
/// Listen on a TCP socket and forward incoming requests as IncomingCommand.
pub async fn serve_tcp(
    ...
    info!(?addr, "daemon tcp rpc socket listening");

    loop {
        tokio::select! {
            _ = cancel.cancelled() => {
                info!("tcp rpc listener shutting down");
                break;
            }
            accept = listener.accept() => {
                ...
                let tx = tx.clone();
                tokio::spawn(async move {
                    if let Err(e) = handle_tcp_connection(stream, tx).await {
                        warn!(?e, ?peer, "tcp rpc client connection error");
                    }
                });
            }
        }
