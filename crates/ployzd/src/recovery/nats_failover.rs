...
pub(crate) fn mirrored_server_pool(mirror: &IntentMirror, seed: &NatsClientUrl) -> Vec<String> {
    match mirror.load() {
...
impl IntentFailoverRun {
    async fn apply_snapshot(&mut self, snapshot: &IntentSnapshot) {
        ...
        match outcome {
            ...
            IntentMirrorStoreOutcome::RejectedStale => self.enforce_pool_over_stale(snapshot).await,
        }
    }
...
mod tests {
    ...
    async fn shutdown_cancels_blocked_intent_repair() {
        ...
        let task = spawn_intent_failover_mirror(
            client,
            IntentFailover {
                mirror: IntentMirror::new(dir.path().join("intent-mirror.json")),
