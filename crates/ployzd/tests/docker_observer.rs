...
use ployzd::node_agent::observer::{InMemoryObservationStore, ObservationStoreError};
...
fn docker_event_updates_latest_container_observation() {
    ...
    store.put_container(observation.clone());
...
fn full_sync_corrects_missed_docker_event() {
    let mut store = InMemoryObservationStore::new();
    store.put_container(managed_observation(
        "ctr_123",
    ...
    store
        .replace_node_containers(&node_id("node_7"), [exited.clone()])
...
fn full_sync_removes_stale_containers_for_the_node() {
    ...
    store.put_container(retained.clone());

    store
        .replace_node_containers(&node_id("node_7"), [retained.clone()])
        .expect("matching node snapshot");
...
#[test]
fn full_sync_rejects_observations_for_a_different_node_without_mutating() {
    let mut store = InMemoryObservationStore::new();
    let original = managed_observation("ctr_123", ContainerRuntimeState::Running);
    store.put_container(original.clone());

    let mut wrong_node = managed_observation("ctr_456", ContainerRuntimeState::Running);
    wrong_node.node_id = node_id("node_8");

    assert_eq!(
        store.replace_node_containers(&node_id("node_7"), [wrong_node]),
        Err(ObservationStoreError::SnapshotNodeMismatch {
...
fn observation_key_matches_kv_obs_container_path() {
    assert_eq!(
        ContainerObservationKey::from_ids(&node_id("node_7"), &container_id("ctr_123")).as_str(),
        "containers.node_7.ctr_123"
    );
}
