...
pub const MACHINE_STORAGE_PREPARE_CANCEL_ACK_BUDGET: Duration =
    Duration::from_secs(MACHINE_STORAGE_PREPARE_TERMINATION_GRACE_SECONDS * 2 + 1);
pub const MACHINE_STORAGE_PREPARE_CANCEL_RPC_TIMEOUT: Duration =
    Duration::from_secs(MACHINE_STORAGE_PREPARE_TERMINATION_GRACE_SECONDS * 2 + 5);
pub const MACHINE_STORAGE_PREPARE_CANCEL_API_TIMEOUT: Duration =
    Duration::from_secs(MACHINE_STORAGE_PREPARE_TERMINATION_GRACE_SECONDS * 2 + 10);
...
pub enum StorageEffectFailure {
    ...
    #[serde(alias = "sparse_pool")]
    OwnedPool { message: String },
    #[error("Ployz owned pool backing evidence already exists: {}", backing_file.display())]
    OwnedPoolEvidencePresent { backing_file: PathBuf },
    #[error(
    ...
    #[error(
        "Ployz owned pool allocation did not preserve host headroom: available={available_bytes} required_headroom={required_headroom_bytes}"
    )]
    OwnedPoolHeadroomNotPreserved {
        available_bytes: u64,
        required_headroom_bytes: u64,
    },
