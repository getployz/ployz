use ployz_core::deploy::{
    ...
    ImageReference, ReplicaCount, ReplicaSlot, plan_new_service_deploy,
};
...
fn new_service_plan_runs_replicas_across_eligible_nodes() {
    assert_eq!(
        plan_new_service_deploy(planning_input(3, [node_id("node_a"), node_id("node_b")]))
...
fn deploy_plan_requires_eligible_node() {
    assert_eq!(
        plan_new_service_deploy(planning_input(1, [])),
