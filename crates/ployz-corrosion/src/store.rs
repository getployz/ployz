...
use ployz_sdk::model::{
    ...
    OverlayIp, Participation, PublicKey, RoutingState, ServiceHeadRecord, ServiceRevisionRecord,
...
impl CorrosionStore {
    ...
    pub async fn connect_for_network(data_dir: &Path, network: &str) -> Result<Self> {
        ...
        let raw = std::fs::read_to_string(&network_path).map_err(|e| {
            Error::operation(
                "connect_for_network",
                format!("reading network config from {}: {e}", network_path.display()),
            )
        })?;
        ...
        let network_config: NetworkConfigMinimal = serde_json::from_str(&raw).map_err(|e| {
            Error::operation("connect_for_network", format!("parsing network config: {e}"))
        })?;
    ...
    /// Opens 4 subscriptions (revisions, heads, slots, instances).
    /// Returns initial `RoutingState` from Row events, then spawns a task
    /// that applies Change deltas and sends updated `RoutingState` through the channel.
    pub async fn subscribe_routing_state(
        &self,
    ) -> Result<(RoutingState, mpsc::Receiver<RoutingState>)> {
        subscribe_routing_state_inner(&self.client).await
    }
}
...
// ---------------------------------------------------------------------------
// subscribe_routing_state — subscription-based live routing state
// ---------------------------------------------------------------------------

use crate::client::SubscriptionStream;
use corro_api_types::RowId;
...
/// Collect initial snapshot rows from a subscription until EndOfQuery.
    ...
    loop {
        ...
        match event {
            ...
            TypedQueryEvent::Row(rowid, cells) => {
                ...
                let k = key(&record);
                row_index.insert(rowid.0, k.clone());
                map.insert(k, record);
            }
            TypedQueryEvent::Change(ct, rowid, cells, _) => {
                // Changes during initial query phase — apply as upsert/delete
                apply_routing_change(ct, rowid, &cells, parse, key, map, row_index)?;
            }
        }
    }
}
    ...
    K: Eq + std::hash::Hash + Clone,
{
    match ct {
        ...
        ChangeType::Delete => {
            if let Some(k) = row_index.remove(&rowid.0) {
                map.remove(&k);
            }
        }
    }
...
async fn subscribe_routing_state_inner(
    client: &CorrClient,
) -> Result<(RoutingState, mpsc::Receiver<RoutingState>)> {
    let mut state = LiveRoutingState::new();

    // Open 4 subscriptions with skip_rows=false to get initial snapshot
    let rev_stmt = Statement::Simple(SQL_ALL_SERVICE_REVISIONS.to_string());
    let head_stmt = Statement::Simple(SQL_ALL_SERVICE_HEADS.to_string());
    let slot_stmt = Statement::Simple(SQL_ALL_SERVICE_SLOTS.to_string());
    let inst_stmt = Statement::Simple(SQL_ALL_INSTANCE_STATUS.to_string());

    let mut rev_stream = client
        .subscribe(&rev_stmt, false, None)
        .await
        .map_err(|e| Error::operation("subscribe_routing_state", format!("revisions: {e}")))?;
    let mut head_stream = client
        .subscribe(&head_stmt, false, None)
        .await
        .map_err(|e| Error::operation("subscribe_routing_state", format!("heads: {e}")))?;
    let mut slot_stream = client
        .subscribe(&slot_stmt, false, None)
        .await
        .map_err(|e| Error::operation("subscribe_routing_state", format!("slots: {e}")))?;
    let mut inst_stream = client
        .subscribe(&inst_stmt, false, None)
        .await
        .map_err(|e| Error::operation("subscribe_routing_state", format!("instances: {e}")))?;

    // Collect initial snapshot from all 4 streams
    collect_initial_rows(
        &mut rev_stream,
        &parse_service_revision,
        &|r: &ServiceRevisionRecord| {
            (r.namespace.clone(), r.service.clone(), r.revision_hash.clone())
        },
        &mut state.revisions,
    ...
    collect_initial_rows(
        &mut head_stream,
        &parse_service_head,
        &|r: &ServiceHeadRecord| (r.namespace.clone(), r.service.clone()),
        &mut state.heads,
        &mut state.head_rows,
        "heads",
    )
    ...
    collect_initial_rows(
        ...
        &parse_service_slot,
        &|r: &ServiceSlotRecord| {
            (r.namespace.clone(), r.service.clone(), r.slot_id.clone())
        },
        &mut state.slots,
        &mut state.slot_rows,
        "slots",
    ...
    collect_initial_rows(
        &mut inst_stream,
        &parse_instance_status,
        &|r: &InstanceStatusRecord| r.instance_id.clone(),
        &mut state.instances,
    ...
    // Spawn background task that processes Change events from all 4 streams
    tokio::spawn(async move {
        let mut dirty = false;
        ...
        loop {
            tokio::select! {
                event = rev_stream.next() => {
                    let Some(event) = event else { return; };
                    match event {
                        Ok(TypedQueryEvent::Change(ct, rowid, cells, _)) => {
                            let _ = apply_routing_change(
                                ct, rowid, &cells,
                                &parse_service_revision,
                                &|r: &ServiceRevisionRecord| (r.namespace.clone(), r.service.clone(), r.revision_hash.clone()),
                                &mut state.revisions, &mut state.revision_rows,
                            );
                            dirty = true;
                        }
                        Ok(_) => {}
                        Err(e) => { warn!(?e, "revision subscription error"); }
                    }
                }
                    ...
                    match event {
                        Ok(TypedQueryEvent::Change(ct, rowid, cells, _)) => {
                            let _ = apply_routing_change(
                                ...
                                &parse_service_head,
                                &|r: &ServiceHeadRecord| (r.namespace.clone(), r.service.clone()),
                                &mut state.heads, &mut state.head_rows,
                            );
                            dirty = true;
                        }
                        Ok(_) => {}
                        Err(e) => { warn!(?e, "head subscription error"); }
                    }
                }
                    ...
                    match event {
                        Ok(TypedQueryEvent::Change(ct, rowid, cells, _)) => {
                            let _ = apply_routing_change(
                                ct, rowid, &cells,
                                &parse_service_slot,
                                &|r: &ServiceSlotRecord| (r.namespace.clone(), r.service.clone(), r.slot_id.clone()),
                                &mut state.slots, &mut state.slot_rows,
                            );
                            dirty = true;
                        }
                        Ok(_) => {}
                        Err(e) => { warn!(?e, "slot subscription error"); }
                    }
                }
                event = inst_stream.next() => {
                    let Some(event) = event else { return; };
                    match event {
                        Ok(TypedQueryEvent::Change(ct, rowid, cells, _)) => {
                            let _ = apply_routing_change(
                        ...
                        Ok(_) => {}
                        Err(e) => { warn!(?e, "instance subscription error"); }
                    }
                }
            ...
            // Reset debounce timer when newly dirty
            if dirty {
                debounce.as_mut().reset(tokio::time::Instant::now() + DEBOUNCE_WINDOW);
            }
        }
