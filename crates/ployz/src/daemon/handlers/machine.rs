...
const MACHINE_ADD_BOOTSTRAP_COMMAND: &str = "set -eu; command -v ployzd >/dev/null 2>&1 || { echo 'ployzd not installed'; exit 12; }; command -v ployz >/dev/null 2>&1 || { echo 'ployz not installed'; exit 13; }; ployz status >/dev/null 2>&1 || { echo 'ployzd not running'; exit 15; };";
const REMOTE_MESH_INIT_COMMAND: &str = "set -eu; ployz mesh init --name-stdin";
const REMOTE_MESH_JOIN_COMMAND: &str = "set -eu; ployz mesh join --token-stdin";
const REMOTE_MESH_SELF_RECORD_COMMAND: &str = "set -eu; ployz mesh self-record";
const REMOTE_MESH_READY_COMMAND: &str = "set -eu; ployz mesh ready --json";
const REMOTE_MESH_DOWN_COMMAND: &str = "set -eu; ployz mesh down";
const REMOTE_MESH_DESTROY_COMMAND: &str = "set -eu; ployz mesh destroy --name-stdin";
