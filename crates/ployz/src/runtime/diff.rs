use super::labels::{LABEL_KEY, LABEL_KIND, LABEL_MANAGED, LABEL_PARENT_ID, LEGACY_LABEL_CONFIG_HASH};
...
#[must_use]
pub fn parent_id_matches(
    observed: &ObservedContainer,
    desired_parent_id: Option<&str>,
) -> bool {
...
mod tests {
    ...
    fn old_labels_only_treated_as_legacy() {
        let mut observed = base_observed();
        observed.labels.insert("ployz.config-hash".into(), "abc".into());
