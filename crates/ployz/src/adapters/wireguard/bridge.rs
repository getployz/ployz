...
async fn bridge_event_loop(
    ...
    if let TunnResult::WriteToNetwork(data) = tunn.format_handshake_initiation(&mut handshake_buf, false) {
    ...
    loop {
        tokio::select! {
            ...
            result = udp.recv_from(&mut udp_recv_buf) => {
                    ...
                    TunnResult::WriteToNetwork(data) => {
                        ...
                        loop {
                            match tunn.decapsulate(None, &[], &mut extra_buf) {
                                ...
                                TunnResult::Done
                                | TunnResult::Err(_)
                                | TunnResult::WriteToTunnelV4(..) => break,
                            }
                        }
                    }
                    TunnResult::WriteToTunnelV6(data, _) => {
                ...
                while let Some(ip_packet) = device.pop_tx() {
                    if let TunnResult::WriteToNetwork(data) = tunn.encapsulate(&ip_packet, &mut wg_scratch) {
                        diag.mark_data_send();
                        let _ = udp.send_to(data, peer_endpoint).await;
                    }
                }
