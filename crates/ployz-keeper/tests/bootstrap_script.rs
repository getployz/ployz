...
#[cfg(unix)]
#[test]
fn bootstrap_script_installs_keeper_by_default_from_alpha_channel() {
    let root = temp_dir("ployz-bootstrap-script-keeper");
    let keeper_source = write_fake_keeper(&root);
    ...
    assert!(stdout.contains(&format!(
        "installed {}",
        install_dir.join("ployz-keeper").display()
    )));
    assert!(stdout.contains("run: sudo ployz-keeper bootstrap"));
...
fn bootstrap_script_reports_missing_manifest_key_and_url() {
    ...
    assert_stderr_contains(
        &output,
        "release manifest https://github.com/getployz/ployz/releases/download/v0.0.2-alpha.1/ployz-release-linux-amd64.env is missing PLOYZ_KEEPER_URL",
    );
}
...
fn write_fake_keeper(root: &std::path::Path) -> PathBuf {
    let keeper = root.join("ployz-keeper-source");
    fs::write(&keeper, "#!/bin/sh\nexit 0\n").expect("fake keeper source can be written");
    keeper
}
...
fn write_keeper_manifest(
    path: &std::path::Path,
    platform: &str,
    tag: &str,
    version: &str,
    keeper_source: &std::path::Path,
) {
    fs::write(
        path,
        format!(
            "PLOYZ_VERSION={version}\nPLOYZ_RELEASE_TAG={tag}\nPLOYZ_RELEASE_PLATFORM={platform}\nPLOYZ_KEEPER_URL=file://{}\nPLOYZ_KEEPER_SHA256={KEEPER_DIGEST}\n",
            keeper_source.display()
        ),
    )
    .expect("manifest can be written");
}
