...
use ployz_keeper::cli::KeeperCorePromote;
use ployz_keeper::command::SystemKeeperCommandRunner;
use ployz_keeper::core_demote::repoint_non_core_roles;
use ployz_keeper::executor::{KeeperPlanTerminal, execute_keeper_plan};
...
use ployz_keeper::release_manifest::release_manifest_url;
use ployz_keeper::report::KeeperTextRecorder;
use ployz_keeper::steps::{CorePromoteTarget, core_promote_plan};

use crate::env_config::{
    default_machine_join_template_file, load_versioned_release_manifest, local_release_manifest_url,
};
use crate::runtime::{
    CORE_PROMOTE_RESULT_BEGIN, CORE_PROMOTE_RESULT_END, KEEPER_STATE_DIR, failure_summary,
};

pub(crate) fn run_core_promote_command(promote: KeeperCorePromote) -> ExitCode {
    if promote.check {
