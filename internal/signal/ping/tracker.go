...
func NewTracker() *Tracker {
	return &Tracker{rtts: make(map[string]Sample)}
}

func (pt *Tracker) Run(ctx context.Context, selfID string, interval time.Duration, resolveAddrs func() map[string]string) {
...
func (pt *Tracker) probeAll(ctx context.Context, selfID string, addrs map[string]string) {
	type result struct {
		nodeID string
		sample Sample
	}
...
func (pt *Tracker) dial(ctx context.Context, addr string) Sample {
	if pt.DialFunc != nil {
...
func (pt *Tracker) Snapshot() map[string]Sample {
	pt.mu.RLock()
	defer pt.mu.RUnlock()
...
func (pt *Tracker) Remove(nodeID string) {
	pt.mu.Lock()
