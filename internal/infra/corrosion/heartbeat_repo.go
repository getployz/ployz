...
func (r HeartbeatRepo) SubscribeHeartbeats(ctx context.Context) ([]overlay.HeartbeatRow, <-chan overlay.HeartbeatChange, error) {
...
func decodeHeartbeatRow(values []json.RawMessage) (overlay.HeartbeatRow, error) {
	if len(values) != 3 {
		return overlay.HeartbeatRow{}, fmt.Errorf("decode heartbeat row: expected 3 columns, got %d", len(values))
	}

	var out overlay.HeartbeatRow
	var err error
	if out.NodeID, err = decodeString(values[0], "heartbeat node_id"); err != nil {
		return overlay.HeartbeatRow{}, err
	}
