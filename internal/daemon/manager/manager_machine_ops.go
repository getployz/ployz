...
func (m *Manager) GetIdentity(_ context.Context) (types.Identity, error) {
	...
	st, err := overlay.LoadState(m.stateStore, cfg)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return types.Identity{}, fmt.Errorf("network %q: %w", spec.Network, overlay.ErrNotInitialized)
		}
		return types.Identity{}, err
	}
	...
	return types.Identity{
		...
		Running:             st.Phase == overlay.NetworkRunning,
	}, nil
}
...
func (m *Manager) ListMachines(ctx context.Context) ([]types.MachineEntry, error) {
	...
	for _, row := range rows {
		...
		if ph, ok := health.Peers[row.ID]; ok {
			...
			entry.Stale = ph.Phase == convergence.FreshnessStale
...
func (m *Manager) RemoveMachine(ctx context.Context, idOrEndpoint string) error {
	...
	return m.membership.RemoveMachine(ctx, cfg, idOrEndpoint)
}
