...
func (m *Manager) PlanDeploy(ctx context.Context, network, namespace string, composeSpec []byte) (types.DeployPlan, error) {
	plan, _, err := m.buildDeployPlan(ctx, network, namespace, composeSpec)
	if err != nil {
...
func (m *Manager) ApplyDeploy(ctx context.Context, network, namespace string, composeSpec []byte, events chan<- types.DeployProgressEvent) (types.DeployResult, error) {
	plan, cfg, err := m.buildDeployPlan(ctx, network, namespace, composeSpec)
	if err != nil {
		return types.DeployResult{}, err
	}

	identity, err := m.GetIdentity(ctx, network)
	if err != nil {
	...
	if applyErr != nil {
		...
		if errors.As(applyErr, &deployErr) {
			...
			out.ErrorPhase = deployErr.Phase
...
func (m *Manager) ListDeployments(ctx context.Context, network, namespace string) ([]types.DeploymentEntry, error) {
...
func (m *Manager) RemoveNamespace(ctx context.Context, network, namespace string) error {
	network = defaults.NormalizeNetwork(network)
	namespace = strings.TrimSpace(namespace)
	if network == "" {
...
func (m *Manager) ReadContainerState(ctx context.Context, network, namespace string) ([]types.ContainerState, error) {
	network = defaults.NormalizeNetwork(network)
	namespace = strings.TrimSpace(namespace)
	if network == "" {
		return nil, fmt.Errorf("network is required")
	...
	_, _, err := m.resolveConfig(network)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) buildDeployPlan(ctx context.Context, network, namespace string, composeSpec []byte) (deploy.DeployPlan, netctrl.Config, error) {
	network = defaults.NormalizeNetwork(network)
	namespace = strings.TrimSpace(namespace)
	if network == "" {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("network is required")
	}
	if namespace == "" {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("namespace is required")
	}
	if len(composeSpec) == 0 {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("compose spec is required")
	}

	project, err := deploy.LoadSpec(ctx, composeSpec, namespace)
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("load compose spec: %w", err)
	}
	incoming, err := composeProjectToDeploySpec(project, namespace, network)
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, err
	}
	...
	store := corrosion.NewStore(cfg.CorrosionAPIAddr, cfg.CorrosionAPIToken)
	if err := store.EnsureContainerTable(ctx); err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("ensure deploy container table: %w", err)
	}
	current, err := store.ListContainersByNamespace(ctx, namespace)
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("list containers for namespace %q: %w", namespace, err)
	}

	machines, err := m.ctrl.ListMachines(ctx, cfg)
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("list machines: %w", err)
	}
	...
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("schedule deploy: %w", err)
	}

	plan, err := deploy.PlanDeploy(incoming, current, assignments)
	if err != nil {
		return deploy.DeployPlan{}, netctrl.Config{}, fmt.Errorf("plan deploy: %w", err)
	}
	return plan, cfg, nil
}

func composeProjectToDeploySpec(project *compose.Project, namespace, network string) (deploy.DeploySpec, error) {
...
type runtimeHealthChecker struct {
	runtime netctrl.ContainerRuntime
}
