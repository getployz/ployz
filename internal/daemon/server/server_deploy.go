...
const (
	...
	deployPhaseOwnership     = "ownership"
	deployPhasePrePull       = "pre-pull"
	deployPhaseHealth        = "health"
	deployPhasePostcondition = "postcondition"
)
...
func (s *Server) PlanDeploy(ctx context.Context, req *pb.PlanDeployRequest) (*pb.PlanDeployResponse, error) {
	...
	if req.Network == "" || req.Namespace == "" || len(req.ComposeSpec) == 0 {
		return nil, status.Error(codes.InvalidArgument, "network, namespace, and compose_spec are required")
	}

	plan, err := s.manager.PlanDeploy(ctx, req.Network, req.Namespace, req.ComposeSpec)
	if err != nil {
...
func (s *Server) ApplyDeploy(req *pb.ApplyDeployRequest, stream pb.Daemon_ApplyDeployServer) error {
	...
	if req.Network == "" || req.Namespace == "" || len(req.ComposeSpec) == 0 {
		return status.Error(codes.InvalidArgument, "network, namespace, and compose_spec are required")
	}
	...
	var applyErr error
	go func() {
		result, applyErr = s.manager.ApplyDeploy(stream.Context(), req.Network, req.Namespace, req.ComposeSpec, events)
		close(events)
		close(done)
	}()
				...
				if applyErr != nil {
					...
					if errors.As(applyErr, &deployErr) {
						if result.ErrorPhase == "" {
							result.ErrorPhase = deployErr.Phase
						}
...
func (s *Server) ListDeployments(ctx context.Context, req *pb.ListDeploymentsRequest) (*pb.ListDeploymentsResponse, error) {
	...
	if req.Network == "" || req.Namespace == "" {
		return nil, status.Error(codes.InvalidArgument, "network and namespace are required")
	}

	rows, err := s.manager.ListDeployments(ctx, req.Network, req.Namespace)
	if err != nil {
		return nil, deployErrToStatus(err)
	}
...
func (s *Server) RemoveNamespace(ctx context.Context, req *pb.RemoveNamespaceRequest) (*pb.RemoveNamespaceResponse, error) {
	...
	if err := s.manager.RemoveNamespace(ctx, req.Network, req.Namespace); err != nil {
		return nil, deployErrToStatus(err)
	}
...
func (s *Server) ReadContainerState(ctx context.Context, req *pb.ReadContainerStateRequest) (*pb.ReadContainerStateResponse, error) {
	...
	rows, err := s.manager.ReadContainerState(ctx, req.Network, req.Namespace)
	if err != nil {
		return nil, deployErrToStatus(err)
	}
...
func deployErrToStatus(err error) error {
	...
	if errors.As(err, &deployErr) {
		...
		case deployPhaseOwnership:
			return status.Error(codes.Aborted, message)
		case deployPhasePrePull:
			return status.Error(codes.Unavailable, message)
		case deployPhaseHealth:
			return status.Error(codes.FailedPrecondition, message)
		case deployPhasePostcondition:
			return status.Error(codes.Aborted, message)
		default:
