...

{
  ...
  printf 'profile=%s\n' "${build_profile}"
  {
    git -C "${repo_root}" ls-files -z -- Cargo.toml Cargo.lock MVP .nats-version ployz.sh crates scripts packaging
    git -C "${repo_root}" ls-files --others --exclude-standard -z -- Cargo.toml Cargo.lock MVP .nats-version ployz.sh crates scripts packaging
  } | LC_ALL=C sort -zu \
