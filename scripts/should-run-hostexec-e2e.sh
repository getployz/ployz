...
for path in "${changed_files[@]}"; do
  case "${path}" in
    .github/workflows/hostexec-e2e.yml|Cargo.toml|Cargo.lock|.corrosion-version|ployz.sh|Dockerfile.e2e-node|scripts/build-install-payload.sh|scripts/install-ebpf-bytecode.sh|packaging/e2e/*|packaging/systemd/*|ebpf/*)
