...
// Service is the overlay capability root.
...
// ListContainers lists containers from the configured runtime.
func (c *Controller) ListContainers(ctx context.Context, labelFilter map[string]string) ([]ContainerListEntry, error) {
	if c == nil || c.containers == nil {
		return nil, fmt.Errorf("container runtime is unavailable")
	}
	return c.containers.ContainerList(ctx, labelFilter)
}
