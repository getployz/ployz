...
func (m *Manager) GetStatus(ctx context.Context) (types.NetworkStatus, error) {
	supervisorPhase, supervisorErr := m.convergence.Status()
	...
	status, err := m.overlay.Status(ctx, cfg)
	if err != nil {
		return types.NetworkStatus{}, err
	}
...
func (m *Manager) GetPeerHealth(ctx context.Context) ([]types.PeerHealthResponse, error) {
	...
	for nodeID, ph := range health.Peers {
		pingRTT := ph.PingRTT
		if ph.PingPhase == convergence.PingUnreachable {
			pingRTT = -1
		}
		peers = append(peers, types.PeerLag{
			NodeID:         nodeID,
			Freshness:      ph.Freshness,
			Stale:          ph.Phase == convergence.FreshnessStale,
...
func clockHealth(ntp convergence.Status) types.ClockHealth {
	return types.ClockHealth{
		NTPOffsetMs: float64(ntp.Offset.Milliseconds()),
		NTPHealthy:  ntp.Phase == convergence.NTPHealthy,
		NTPError:    ntp.Error,
	}
}
