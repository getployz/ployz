...
func Configure(ctx context.Context, iface string, mtu int, privateKey string,
	...
	activeSession = &darwinSession{
		...
		tun:       tunDev,
		ifaceName: tunName,
		mgmtCIDR:  overlay.ManagementCIDR,
	}
...
func syncRoutes(ctx context.Context, iface string, machineIP, mgmtIP netip.Addr, peers []PeerConfig) error {
	...
	_, _ = runPrivilegedCommand(ctx, "route", "-n", "delete", "-inet6", overlay.ManagementCIDR, "-interface", iface)
	if out, err := runPrivilegedCommand(ctx, "route", "-n", "add", "-inet6", overlay.ManagementCIDR, "-interface", iface); err != nil {
