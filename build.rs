fn main() {
    #[cfg(feature = "ebpf-native")]
    {
        ...
        let prebuilt = std::path::Path::new("ebpf/target/bpfel-unknown-none/release/ployz-ebpf-tc");
        if prebuilt.exists() {
            println!("cargo:rerun-if-changed=ebpf/target/bpfel-unknown-none/release/ployz-ebpf-tc");
